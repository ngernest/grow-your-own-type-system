...
and 'a tvar = Unbound of id * level | Link of 'a ty | Generic of id
...
let rec strip_refined_types = function
  ...
  | TApp (name, arg_ty_list) ->
    TApp (name, List.map strip_refined_types arg_ty_list)
...
let rec duplicate_without_refined_types = function
  (* Removes refined types, duplicating the type (including the type
     variables). *)
