...
let primitives =
  [ ("get", "forall[t] (a : array[t], i : int if i >= 0 and i < length(a)) -> t");
    ...
    ( "memcpy",
      "(dst : array[byte], src : array[byte],\n\
       \t\t\t           num : int if num <= length(dst) and num <= \
       length(src)) -> unit" );
    ("head", "forall[t] (a : array[t] if length(a) > 0) -> t");
    ( "is_empty",
      "forall[t] (a : array[t]) -> (b : bool if b == (length(a) == 0))" );
    ...
    ( "choose_curry",
      "forall[a] (x : a) -> (y : a) -> (z : a if z == x or z == y)" );
...
let env =
  let f env (var_name, ty_str) =
    ...
    Env.extend var_name t_ty env in
  List.fold_left f Env.empty
    (List.flatten [ builtins; uninterpreted; primitives ])
