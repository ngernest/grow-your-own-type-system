  ...
  | Some (pid, c_in, c_out) ->
    if Unix.getpid () = pid then (
      ...
      | Unix.WEXITED exit_code ->
        Printf.printf "Z3 exited with exit code %i\n" exit_code
      | Unix.WSIGNALED signal ->
        Printf.printf "Z3 was killed by a signal %i\n" signal
      | Unix.WSTOPPED signal ->
        Printf.printf "Z3 was stopped by a signal %i\n" signal);
      if process_status <> Unix.WEXITED 0 || log_Z3_input then (
        print_endline "\n\nZ3 LOG\n";
        print_endline !log);
