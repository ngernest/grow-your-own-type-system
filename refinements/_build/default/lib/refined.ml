...
module LocalEnv = struct
  ...
  let extend name ty env =
    ...
    else StringMap.add name ty env

  let lookup name env = try StringMap.find name env with Not_found -> name
end
...
module FnEnv = struct
  ...
  let extend name local_env_and_ty env =
    if StringMap.mem name env then
      error ("duplicate variable name \"" ^ name ^ "\"")
    else StringMap.add name local_env_and_ty env
...
let declare_new_var ty =
  let var_name =
    ...
    | TVar _ -> "v"
    | TArrow _ -> error "cannot declare variables with function types" in
  let var_number =
    try Hashtbl.find var_name_map var_name with Not_found -> 0 in
...
(* let assert_true if_clause x = match if_clause with | None -> Smt.write
   ("(assert " ^ x ^ ")") | Some f -> Smt.write ("(assert (=> " ^ f ^ " " ^ x ^
   "))") *)

let assert_true translated_expr = Smt.write ("(assert " ^ translated_expr ^ ")")

let assert_false translated_expr =
...
let rec check_contract if_clause fn_env local_env contract_expr =
  Smt.push_pop (fun () ->
      ...
      | None -> ());
      assert_false
        (check_value Formula if_clause fn_env local_env contract_expr);
  ...
  Smt.push_pop (fun () ->
          ...
          (fun (closure_local_env, local_env) param_r_ty expected_param_r_ty ->
            let var_name, new_local_env =
              ...
              | Refined (name, ty, expr) ->
                ...
                assert_true
                  (check_value Formula if_clause fn_env new_local_env expr);
                (var_name, new_local_env) in
            let new_closure_local_env =
              match param_r_ty with
              | Plain _ -> closure_local_env
              | Named (name, _) ->
                LocalEnv.extend name var_name closure_local_env
              | Refined (name, _, expr) ->
                let new_closure_local_env =
                  LocalEnv.extend name var_name closure_local_env in
                check_contract if_clause fn_env new_closure_local_env expr;
                new_closure_local_env in
            (new_closure_local_env, new_local_env))
          (closure_local_env, local_env)
          param_r_ty_list expected_param_r_ty_list in
      ...
      | Refined (name, _, expr) ->
        let closure_return_ty_local_env =
          LocalEnv.extend name return_var_name new_closure_local_env in
        assert_true
          (check_value Formula if_clause fn_env closure_return_ty_local_env expr));
      match expected_return_r_ty with
      | Plain _ | Named _ -> ()
      | Refined (name, _, expr) ->
        let return_ty_local_env =
          LocalEnv.extend name return_var_name new_local_env in
...
and check_function_call if_clause fn_env local_env fn_expr arg_expr_list =
  ...
  let param_r_ty_list, return_r_ty =
    ...
    | TArrow (param_r_ty_list, return_r_ty) -> (param_r_ty_list, return_r_ty)
    | _ -> assert false in
  let rev_translated_arg_expr_list, new_closure_local_env =
    List.fold_left2
      (fun (rev_translated_arg_expr_list, closure_local_env) param_r_ty arg_expr ->
        if is_function_ty (plain_ty param_r_ty) then
          error "not implemented - argument is a function"
        else
          let new_closure_local_env, translated_arg_expr =
            match param_r_ty with
            | Plain _ ->
              ( closure_local_env,
                check_value Formula if_clause fn_env local_env arg_expr )
            | Named (name, _) ->
              let translated_arg_expr =
                check_value Term if_clause fn_env local_env arg_expr in
              ( LocalEnv.extend name translated_arg_expr closure_local_env,
                translated_arg_expr )
            | Refined (name, _, expr) ->
              let translated_arg_expr =
                check_value Term if_clause fn_env local_env arg_expr in
              let new_closure_local_env =
                LocalEnv.extend name translated_arg_expr closure_local_env in
              check_contract if_clause fn_env new_closure_local_env expr;
              (new_closure_local_env, translated_arg_expr) in
          ( translated_arg_expr :: rev_translated_arg_expr_list,
            new_closure_local_env ))
      ([], closure_local_env) param_r_ty_list arg_expr_list in
...
and check_value expected_result if_clause fn_env local_env expr =
  ...
  | ECall (({ shape = EVar fn_name; ty = _ } as fn_expr), arg_expr_list)
    when StringSet.mem fn_name builtins || StringSet.mem fn_name uninterpreted
    -> (
    let return_r_ty, translated_arg_expr_list, closure_local_env =
      check_function_call if_clause fn_env local_env fn_expr arg_expr_list in
    let translated_expr =
      translate_builtin_and_uninterpreted fn_name translated_arg_expr_list in
  ...
  | ELet (var_name, value_expr, body_expr)
    when not (is_function_ty value_expr.ty) ->
    let translated_value_expr =
      check_value Formula if_clause fn_env local_env value_expr in
  ...
  | EIf (cond_expr, then_expr, else_expr) -> (
    let translated_cond_expr =
      check_value Term if_clause fn_env local_env cond_expr in
    let then_if_clause, else_if_clause =
      match if_clause with
      | None ->
        (Some translated_cond_expr, Some ("(not " ^ translated_cond_expr ^ ")"))
      | Some translated_old_cond_expr ->
        ( Some
            ("(and " ^ translated_old_cond_expr ^ " " ^ translated_cond_expr
           ^ ")"),
          Some
            ("(and " ^ translated_old_cond_expr ^ " (not "
           ^ translated_cond_expr ^ "))") ) in
    let translated_then_expr =
      check_value Formula then_if_clause fn_env local_env then_expr in
    let translated_else_expr =
      check_value Formula else_if_clause fn_env local_env else_expr in
    let translated_if_expr =
      "(ite " ^ translated_cond_expr ^ " " ^ translated_then_expr ^ " "
      ^ translated_else_expr ^ ")" in
    match expected_result with
  ...
  | ECast (expr, ty, Some contract_expr) ->
    let translated_expr =
      check_value expected_result if_clause fn_env local_env expr in
    check_contract if_clause fn_env local_env contract_expr;
    translated_expr
  | ECast (expr, ty, None) ->
    check_value expected_result if_clause fn_env local_env expr

and check_function if_clause fn_env local_env expr =
  assert (is_function_ty expr.ty);
  match expr.shape with
  | EVar name ->
    (*assert (not ((StringSet.mem name builtins) || (StringSet.mem name
      uninterpreted))) ;*)
    ...
    Smt.push_pop (fun () ->
        let param_r_ty_list =
          List.map
            ...
            param_list in
        let return_r_ty =
          match maybe_return_r_ty with
          | Some (Refined (name, ty, expr)) ->
            let translated_body =
              check_value Term if_clause fn_env local_env body_expr in
            let return_ty_local_env =
              LocalEnv.extend name translated_body local_env in
            check_contract if_clause fn_env return_ty_local_env expr;
            Refined (name, ty, expr)
          | _ ->
            ignore (check_value Formula if_clause fn_env local_env body_expr);
            Plain body_expr.ty in
...
let global_fn_env =
  Infer.Env.fold
    (fun fn_name fn_ty fn_env ->
      if not (is_function_ty fn_ty) then fn_env
      else FnEnv.extend fn_name (LocalEnv.empty, real_ty fn_ty) fn_env)
...
let declare_uninterpreted_function fn_name fn_ty =
  (* Declares an uninterpreted symbol, for example
     ...
     (declare-fun length (Other) Int) (assert (forall ((a Other)) (>= (length a)
     0))) *)
  ...
  | TArrow (param_r_ty_list, return_r_ty) -> (
    ...
    | Refined (name, return_ty, expr) ->
      let translated_param_list_str =
        String.concat " "
          (List.map
             (fun (param_name, translated_param_ty) ->
               "(" ^ param_name ^ " " ^ translated_param_ty ^ ")")
             translated_param_list) in
      let param_name_list_str =
        String.concat " " (List.map fst translated_param_list) in
      let result_str = "(" ^ fn_name ^ " " ^ param_name_list_str ^ ")" in
      let local_env = LocalEnv.extend name result_str LocalEnv.empty in
      let translated_expr =
        check_value Formula None global_fn_env local_env expr in
      Smt.write
...
let start () =
  if not !already_started then (
    ...
    StringSet.iter
      (fun fn_name ->
        declare_uninterpreted_function fn_name
          (Infer.Env.lookup fn_name Core.env))
      uninterpreted;
    StringSet.iter
      (fun name ->
        let ty = Infer.Env.lookup name Core.env in
        if not (is_function_ty ty) then declare_var name ty)
      primitives;
...
let check_expr expr =
  ...
  Smt.push_pop (fun () ->
      ...
      else ignore (check_value Formula None global_fn_env LocalEnv.empty expr))
