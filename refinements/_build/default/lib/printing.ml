...
let string_set_from_list l =
  List.fold_left (fun set el -> StringSet.add el set) StringSet.empty l

let hashtbl_values h = Hashtbl.fold (fun _ value acc -> value :: acc) h []

(* Operators *)
let binary_operators =
  string_set_from_list
    [ "and"; "or"; "<"; ">"; "<="; ">="; "=="; "!="; "+"; "-"; "*"; "/"; "%" ]

let unary_operators = string_set_from_list [ "not"; "unary-" ]
    ...
    | TArrow (param_r_ty_list, return_r_ty) ->
      let string_of_param_r_ty = function
        ...
        | Refined (name, ty, expr) ->
          name ^ " : " ^ complex_ty ty ^ " if " ^ string_of_expr expr in
      let param_r_ty_list_str =
        match param_r_ty_list with
        | [ Plain ty ] -> simple_ty ty
        | _ ->
          "("
          ^ String.concat ", " (List.map string_of_param_r_ty param_r_ty_list)
          ^ ")" in
      let return_r_ty_str =
    ...
    | SFun (param_list, maybe_return_r_ty, body_expr) ->
      let string_of_param = function
        ...
        | name, Some (ty, Some contract_expr) ->
          name ^ " : " ^ string_of_s_ty_ann ty ^ " if "
          ^ complex_expr contract_expr in
      let param_list_str =
        String.concat ", " (List.map string_of_param param_list) in
      let return_r_ty_str =
        ...
        | Some return_r_ty -> (
          ...
          match return_r_ty with
          | Plain ty ->
            if is_function_ty ty then "(" ^ string_of_s_ty_ann ty ^ ")"
            else string_of_s_ty_ann ty
          | Named (name, ty) -> "(" ^ name ^ " : " ^ string_of_s_ty_ann ty ^ ")"
          | Refined (name, ty, expr) ->
            "(" ^ name ^ " : " ^ string_of_s_ty_ann ty ^ " if "
            ^ string_of_s_expr expr ^ ")") in
      "fun(" ^ param_list_str ^ ")" ^ return_r_ty_str ^ " -> "
      ^ complex_expr body_expr
    | SLet (var_name, value_expr, body_expr) ->
      "let " ^ var_name ^ " = " ^ complex_expr value_expr ^ " in "
      ^ complex_expr body_expr
    | SIf (cond_expr, then_expr, else_expr) ->
      "if " ^ complex_expr cond_expr ^ " then " ^ complex_expr then_expr
      ^ " else " ^ complex_expr else_expr
    | SCast (expr, ty, None) -> simple_expr expr ^ " : " ^ string_of_s_ty_ann ty
    | SCast (expr, ty, Some contract_expr) ->
      simple_expr expr ^ " : " ^ string_of_s_ty_ann ty ^ " if "
      ^ complex_expr contract_expr
    | expr -> simple_expr expr
  and simple_expr = function
    | SVar name -> name
    | SBool b -> string_of_bool b
    | SInt i -> string_of_int i
    | SCall (SVar op, arg_expr_list) when StringSet.mem op binary_operators -> (
      match arg_expr_list with
      | [ left_expr; right_expr ] ->
        "(" ^ simple_expr left_expr ^ " " ^ op ^ " " ^ simple_expr right_expr
        ^ ")"
      | _ -> failwith ("binary op " ^ op ^ " expects 2 arguments"))
    | SCall (SVar op, arg_expr_list) when StringSet.mem op unary_operators -> (
      match arg_expr_list with
      | [ expr ] ->
        "(" ^ (if op = "unary-" then "-" else op) ^ " " ^ simple_expr expr ^ ")"
      | _ -> failwith ("unary op " ^ op ^ " expects a single argument"))
    | SCall (fn_expr, arg_expr_list) ->
      simple_expr fn_expr ^ "("
      ^ String.concat ", " (List.map complex_expr arg_expr_list)
      ^ ")"
    | expr -> "(" ^ complex_expr expr ^ ")" in
  complex_expr expr

(* Printing typed expressions *)

(*let string_of_plain_t_ty ty = string_of_s_ty (duplicate_without_refined_types
  ty)*)
...
and string_of_t_expr expr : string =
  let rec complex_expr expr =
    ...
    | EFun (param_list, maybe_return_r_ty, body_expr) ->
      ...
      let return_r_ty_str =
        ...
        | Some return_r_ty -> (
          ...
          match return_r_ty with
          | Plain ty ->
            if is_function_ty ty then "(" ^ string_of_t_ty_ann ty ^ ")"
            else string_of_t_ty_ann ty
          | Named (name, ty) -> "(" ^ name ^ " : " ^ string_of_t_ty_ann ty ^ ")"
          | Refined (name, ty, expr) ->
            "(" ^ name ^ " : " ^ string_of_t_ty_ann ty ^ " if "
            ^ string_of_t_expr expr ^ ")") in
      "fun(" ^ param_list_str ^ ")" ^ return_r_ty_str ^ " -> "
      ^ complex_expr body_expr
    ...
    | ECast (expr, ty, Some contract_expr) ->
      simple_expr expr ^ " : " ^ string_of_t_ty_ann ty ^ " if "
      ^ complex_expr contract_expr
    | _ -> simple_expr expr
  and simple_expr expr =
    match expr.shape with
    | EVar name -> name
    | EBool b -> string_of_bool b
    | EInt i -> string_of_int i
    | ECall ({ shape = EVar op; _ }, arg_expr_list)
      when StringSet.mem op binary_operators -> (
      match arg_expr_list with
      | [ left_expr; right_expr ] ->
        "(" ^ simple_expr left_expr ^ " " ^ op ^ " " ^ simple_expr right_expr
        ^ ")"
      | _ -> failwith ("binary op " ^ op ^ " expects 2 arguments"))
    | ECall ({ shape = EVar op; _ }, arg_expr_list)
      when StringSet.mem op unary_operators -> (
      match arg_expr_list with
      | [ expr ] ->
        "(" ^ (if op = "unary-" then "-" else op) ^ " " ^ simple_expr expr ^ ")"
      | _ -> failwith ("unary op " ^ op ^ " expects a single argument"))
