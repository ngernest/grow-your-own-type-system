...
let rec unify ty1 ty2 =
  if ty1 == ty2 then ()
    ...
    | ( TArrow (param_r_ty_list1, return_r_ty1),
        TArrow (param_r_ty_list2, return_r_ty2) ) ->
    ...
    | _, _ ->
      error
        ("cannot unify types " ^ string_of_t_ty ty1 ^ " and "
       ^ string_of_t_ty ty2)
...
let rec generalize level = function
  ...
  | TApp (name, arg_ty_list) ->
    TApp (name, List.map (generalize level) arg_ty_list)
  | TArrow (param_r_ty_list, return_r_ty) ->
    let g = r_ty_map (generalize level) in
    TArrow (List.map g param_r_ty_list, g return_r_ty)
  | TVar { contents = Link ty } -> generalize level ty
  | (TConst _ | TVar { contents = Generic _ } | TVar { contents = Unbound _ })
    as ty ->
    ty
...
let instantiate level ty =
  ...
  let rec f = function
    ...
    | (TConst _ | TVar { contents = Unbound _ }) as ty -> ty in
  f ty
...
let rec match_fun_ty num_params = function
  | TArrow (param_r_ty_list, return_r_ty) ->
    ...
    else (param_r_ty_list, return_r_ty)
  | TVar ({ contents = Unbound (id, level) } as tvar) ->
    let param_r_ty_list =
      let rec f = function
        | 0 -> []
        | n -> Plain (new_var level) :: f (n - 1) in
      f num_params in
...
let rec infer_expr env level = function
  | SVar name ->
    let ty =
      try instantiate level (Env.lookup name env)
      with Not_found -> error ("variable " ^ name ^ " not found") in
    { shape = EVar name; ty }
  ...
  | SCall (fn_s_expr, arg_s_expr_list) ->
    ...
    let param_r_ty_list, return_r_ty =
      match_fun_ty (List.length arg_s_expr_list) fn_t_expr.ty in
    let arg_t_expr_list =
      List.map2
        (fun param_r_ty arg_s_expr ->
          ...
          arg_t_expr)
        param_r_ty_list arg_s_expr_list in
  ...
  | SLet (var_name, value_s_expr, body_s_expr) ->
    ...
    { shape =
        ELet
          ( var_name,
            { shape = value_t_expr.shape; ty = generalized_ty },
            body_t_expr );
      ty = body_t_expr.ty
    }
  ...
  | SCast (s_expr, s_ty, maybe_contract_s_expr) ->
    ...
    let maybe_contract_t_expr =
      match maybe_contract_s_expr with
      | None -> None
      | Some contract_s_expr -> Some (infer_contract env level contract_s_expr)
    in
    { shape = ECast (t_expr, instantiated_t_ty, maybe_contract_t_expr);
      ty = plain_t_ty
    }

and instantiate_and_infer_ty env level ty =
  infer_ty env level (instantiate level ty)

and infer_ty env level = function
  (* Transforms a s_ty into a t_ty, infering the types of contracts along the
     way. *)
  | TConst name -> TConst name
  | TApp (name, arg_ty_list) ->
    TApp (name, List.map (infer_ty env level) arg_ty_list)
  | TArrow (param_s_r_ty_list, return_s_r_ty) ->
    let new_env, rev_param_t_r_ty_list =
      List.fold_left
...
and infer_function env level s_param_list maybe_return_s_r_ty body_s_expr =
  let new_env, rev_t_param_list, rev_param_t_r_ty_list =
    List.fold_left
      (fun (env, rev_t_param_list, rev_param_t_r_ty_list) s_param ->
        let new_env, t_param, param_t_ty =
          match s_param with
          | param_name, None ->
          ...
          | param_name, Some (param_s_ty, None) ->
            ...
            let new_env =
              Env.extend param_name (strip_refined_types param_t_ty) env in
            (new_env, (param_name, param_t_ty, None), param_t_ty)
        ...
        in
        ( new_env,
          t_param :: rev_t_param_list,
          Plain param_t_ty :: rev_param_t_r_ty_list ))
      (env, [], []) s_param_list in
  let t_param_list = List.rev rev_t_param_list in
  let param_t_r_ty_list = List.rev rev_param_t_r_ty_list in
  let body_t_expr = infer_expr new_env level body_s_expr in
  let maybe_return_t_r_ty =
    match maybe_return_s_r_ty with
    | None -> None
    | Some s_r_ty ->
      ...
      let t_r_ty =
        ...
        | Refined (name, _, s_expr) ->
          ...
          Refined (name, instantiated_t_ty, t_expr) in
      Some t_r_ty in
