...
let test_cases =
  [ ("", Fail);
    ...
    ( "let f = fun(x, y : int) -> g(x > -2, y + 1) in f(a, b)",
      OK
        (SLet
           ( "f",
             SFun
               ( [ ("x", None); ("y", Some (TConst "int", None)) ],
                 None,
                 SCall
                   ( SVar "g",
                     [ SCall
                         ( SVar ">",
                           [ SVar "x"; SCall (SVar "unary-", [ SInt 2 ]) ] );
                       SCall (SVar "+", [ SVar "y"; SInt 1 ])
                     ] ) ),
             SCall (SVar "f", [ SVar "a"; SVar "b" ]) )) );
    ( "let x = a : array[byte] in let y = b : int if b > 0 in f(x, y)",
      OK
        (SLet
           ( "x",
             SCast (SVar "a", TApp ("array", [ TConst "byte" ]), None),
             SLet
               ( "y",
                 SCast
                   ( SVar "b",
                     TConst "int",
                     Some (SCall (SVar ">", [ SVar "b"; SInt 0 ])) ),
    ...
    ( "a : int if a > 0",
      OK
        (SCast
           ( SVar "a",
             TConst "int",
             Some (SCall (SVar ">", [ SVar "a"; SInt 0 ])) )) );
    ( "1 : int if a > 0",
      OK
        (SCast
           (SInt 1, TConst "int", Some (SCall (SVar ">", [ SVar "a"; SInt 0 ]))))
    );
    ("f x", Fail);
    ("f x", Fail);
    ("let a = one", Fail);
    ...
    ( "(a and b) or c",
      OK
        (SCall
           (SVar "or", [ SCall (SVar "and", [ SVar "a"; SVar "b" ]); SVar "c" ]))
    );
    ("not a or b", Fail);
    ( "(not a > 0) or b",
      OK
        (SCall
           ( SVar "or",
             [ SCall (SVar "not", [ SCall (SVar ">", [ SVar "a"; SInt 0 ]) ]);
               SVar "b"
             ] )) );
    ...
    ( "1 : some[a] array[a]",
      OK (SCast (SInt 1, TApp ("array", [ bound 0 ]), None)) );
        ...
        (SCast
           ( SInt 1,
             TApp
               ( "pair",
                 [ bound 0;
                   TApp ("pair", [ bound 1; TApp ("array", [ bound 0 ]) ])
                 ] ),
             None )) );
    ( "id : some[a] a -> a",
      OK
        (SCast (SVar "id", TArrow ([ Plain (bound 0) ], Plain (bound 0)), None))
    );
    ( "fun(x : some[a] a, y : int -> int, z : (int -> int) if z(0) != 1) : \
       some[a] array[a] -> 1",
      OK
        (SFun
           ( [ ("x", Some (bound 0, None));
               ( "y",
                 Some
                   ( TArrow ([ Plain (TConst "int") ], Plain (TConst "int")),
                     None ) );
               ( "z",
                 Some
                   ( TArrow ([ Plain (TConst "int") ], Plain (TConst "int")),
                     Some
                       (SCall
                          (SVar "!=", [ SCall (SVar "z", [ SInt 0 ]); SInt 1 ]))
                   ) )
             ],
             Some (Plain (TApp ("array", [ bound 1 ]))),
             SInt 1 )) );
                 ...
                 Some
                   ( TArrow
                       ( [ Refined
                             ( "x",
                               TConst "int",
                               SCall (SVar "!=", [ SVar "x"; SInt 0 ]) )
                         ],
                         Refined
                           ( "y",
                             TConst "int",
                             SCall (SVar "!=", [ SVar "x"; SVar "y" ]) ) ),
                     None ) )
             ],
             None,
             SCall (SVar "f", [ SInt 0 ]) )) );
      ...
      OK
        (SCast
           ( SVar "b",
             TApp ("array", [ bound 0 ]),
             Some
               (SCall (SVar ">", [ SCall (SVar "length", [ SVar "b" ]); SInt 0 ]))
           )) );
    ( "plus : (x : int, y : int) -> (z : int if z == x + y)",
      OK
        (SCast
           ( SVar "plus",
             TArrow
               ( [ Named ("x", TConst "int"); Named ("y", TConst "int") ],
                 Refined
                   ( "z",
                     TConst "int",
                     SCall
                       ( SVar "==",
                         [ SVar "z"; SCall (SVar "+", [ SVar "x"; SVar "y" ]) ]
                       ) ) ),
             None )) );
      ...
      OK
        (SCast
           ( SVar "f",
             TArrow
               ( [ Refined
                     ("x", TConst "int", SCall (SVar ">", [ SVar "x"; SInt 0 ]))
                 ],
                 Plain
                   (TArrow
                      ( [ Named ("y", TConst "int") ],
           ...
           ( [ ( "f",
                 Some
                   ( TArrow
                     ...
                     Some
                       (SCall
                          (SVar ">", [ SCall (SVar "f", [ SInt 0 ]); SInt 1 ]))
                   ) )
             ],
             None,
             SInt 1 )) )
