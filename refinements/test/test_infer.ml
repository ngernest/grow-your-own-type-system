...
type result = OK of string | Fail of string option
...
let test_infer =
  [ (* Hindley-Milner *)
    ...
    ( "let g = fun(f : int -> int if f(true) == 1) -> 1 in g",
    ...
    ( "fun(x : some[a] a if x, y : some[a] a) : (z : bool if y) -> x",
      OK " (bool, bool) -> bool" );
    ( "fun(a) : (f : int -> int if f(a) == 1) -> fun b -> 1",
      OK "int -> int -> int" );
    ("let const_1 = make_const(1) in const_1", OK "forall[a] a -> int");
    (* This one ideally shouldn't fail, but this system doesn't permit duplicate
       variables. *)
...
let test_infer_and_syntax =
  [ (* Function string_of_t_expr prints a typed syntax tree where all function
       arguments already have known types, so in all these test cases all
       parameters must have fully annotated types. *)
    ("fun(x : int) -> x + 1", OK "int -> int");
    ("fun() : ((x : int if x > 0) -> int) -> id", OK "() -> int -> int");
    ("fun() : (x : int if x > 0) -> 1", OK "() -> int");
    ( "fun(a : int) : (f : int -> int if f(a) == 1) -> fun(b : int) -> 1",
      OK "int -> int -> int" )
  ]
...
let normalize ty_str =
  string_of_s_ty (Parser.ty_forall_eof Lexer.token (Lexing.from_string ty_str))
...
let make_single_test_case check_typed_syntax (code, expected_result) =
  ...
  let result, maybe_t_expr =
    ...
    with Infer.Error msg -> (Fail (Some msg), None) in
  assert_equal ~printer:string_of_result ~cmp:cmp_result expected_result result;
  if check_typed_syntax then
    match maybe_t_expr with
    | Some t_expr -> (
      let t_expr_str = string_of_t_expr t_expr in
      Infer.reset_id ();
      try
        let new_s_expr =
          Parser.expr_eof Lexer.token (Lexing.from_string t_expr_str) in
