...
type result = OK | Fail of string option
...
let test_cases =
  [ ("1 : int if true", OK);
    ...
    ( "let x = random1toN(1000) in " ^ " let z = " ^ "  if x > 10 then "
      ^ "   let n = random1toN(x) in " ^ "   n - 2 * x " ^ "  else "
      ^ "   let m = random1toN(10) + 10 in " ^ "   m - x " ^ " in " ^ " 1 / z",
      OK );
    ( "let x = random1toN(100) - 10 in let y = random1toN(100) - 50 in "
      ^ "if x > 0 then " ^ " if y > 0 then " ^ "  1 / x + 1 / y " ^ " else "
      ^ "  1 / x + y " ^ "else " ^ " x + y",
      OK );
    ("fun (x : int) -> 1 / x", wrong);
    ...
    ( "fun (x : int, y : int) : (z : int if z >= x and z >= y) -> if x <= y \
       then x else y",
      wrong );
    ( "fun (x : int, y : int) : (z : int if z >= x and z >= y) -> if x >= y \
       then x else y",
    ...
    ( "fun(x : int if x >= 0) -> let y = x + 9 in let z = square(y) in 1 / (z \
       - random1toN(99))",
      wrong );
    ( "fun(x : int if x >= 0) -> let y = x + 10 in let z = square(y) in 1 / (z \
       - random1toN(99))",
      OK );
    ( "fun(a, def) -> let l = length(a) + 1 in if l >= 1 then get(a, 0) else def",
      wrong );
    ( "fun(a, def) -> let l = length(a) + 1 in if l >= 2 then get(a, 0) else def",
      OK );
    ("fun(a) -> head(a)", wrong);
    ...
    ( "let f = choose_curry(2) in " ^ "let a = f(3) + f(5) in "
    ...
    ( "fun(x : int if x > 0, y : int if y > 0, z : int if z > 0) -> 1 / (x*x*x \
       + y*y*y - z*z*z)",
      unknown );
    ...
    ( "fun(n : int if n >= 0, m : int if m >= 0, "
      ...
      ^ "    a : array[byte] if length(a) == m * n) -> " ^ " get(a, i * n + j)",
      OK );
    (* Heartbleed *)
    ...
    ( "fun(payload : array[byte], "
      ^ "    payload_length : int if length(payload) == payload_length) : \
         array[byte] -> " ^ " let response = alloc(payload_length) in "
      ^ " let ignore = memcpy(response, payload, payload_length) in "
      ^ " response",
      OK );
    ...
    ( "let f = fun(x : int if x > 0) : (y : int if y > 0) -> x + 1 in 1 / f(1)",
      OK );
    ( "let f = fun(x : int if x > 0) : (y : int if y > 0) -> x + 1 in 1 / \
       (f(1) - 1)",
      wrong );
    ( "let f = fun(x : int if x > 0) : (y : int if y > 0) -> x + 1 in f(0)",
      wrong );
    ...
    ( "let min = fun(x, y) : (z : int if z == (if x > y then y else x)) -> "
      ...
      ^ "let abs= fun(i) : (n : int if (if i > 0 then n == i else n == -i)) -> \
         -min(-i, i) in " ^ "abs(-3)",
      OK );
    ( "let max = fun(x, y) : (z : int if (if x > y then z == x else z == y)) -> "
      ^ " if x > y then x else y " ^ "in "
      ^ "let abs = fun(x) : (z : int if z == (if x >= 0 then x else -x)) -> \
         max(x, -x) in " ^ "fun (x : int if abs(x) <= 10) -> "
      ^ " let z = if max(square(x), 25) == 25 then "
      ^ "   3 * x + 7 * random1toN(10) " ^ "  else if x == 11 then " ^ "   0 "
      ^ "  else " ^ "   x " ^ " in " ^ " 100 / z",
      OK );
    ( "let max = fun(x, y) : (z : int if (if x > y then z == x else z == y)) -> "
      ^ " if x < y then x else y " ^ "in max",
      wrong );
    ( "let max = fun(x, y) : (z : int if (if x > y then z == x else z == y)) -> "
      ^ " if x > y then x else y "
      ^ "in 1 : int if max(-7, 3) == 3 and max(15, 3) != 3",
      OK );
    ...
    ( "let const_7 = make_const(7) in fun(x, y) -> 1 : int if const_7(x) == \
       const_7(y)",
      OK );
    ( "let test = fun(x : int, y : int if y == x - 1) -> 1 in "
      ^ "let const_2 = make_const(2) in " ^ "let a = 3 in "
      ^ "test(a, const_2(a))",
      OK );
    ("let f = fun(z, y : int if y == z + 1) -> 1 in let x = 2 in f(1, x)", OK);
    ("let f = fun(x, y : int if y == x + 1) -> 1 in let x = 2 in f(1, x)", OK);
    ( "let test = fun(i : int if i > -6) : (j : int if j >= i * 3) -> i * 3 + \
       1 in " ^ "test : (x : int if x > 0) -> (y : int if y > x)",
      OK );
    ( "let test = fun(i : int if i > 1) : (j : int if j >= i * 3) -> i * 3 + 1 \
       in " ^ "test : (x : int if x > 0) -> (y : int if y > x)",
      wrong );
    ( "let test = fun(i : int if i > -6) : (j : int if j >= i * 3) -> i * 3 + \
       1 in " ^ "test : (x : int if x >= -1) -> (y : int if y > x)",
      wrong );
    (* function subtyping *)
    ...
    ( "let f = fac : (x : int if x > 0) -> (z : int if z > -2) in 1 / f(100)",
      wrong );
    ("fac(0)", OK);
    ("fac(-1)", wrong);
    ("let f = fac in f(0)", OK);
    ...
    ( "fun x -> " ^ "if x >= 0 then "
      ^ " let f = fac : (a : int if a >= x) -> int in " ^ " f(x + 1) " ^ "else "
...
let cmp_result result1 result2 =
...
let make_single_test_case (code, expected_result) =
  ...
  let result =
    try
      ...
      OK
    with Refined.Error msg -> Fail (Some msg) in
  assert_equal ~printer:string_of_result ~cmp:cmp_result expected_result result

let suite = "test_prove" >::: List.map make_single_test_case test_cases
